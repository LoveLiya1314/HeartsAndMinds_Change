...
btc_p_slot_isSaved = "btc_p_slot_isSaved" call BIS_fnc_getParamValue isEqualTo 1;//在断开连接之间保存槽信息
...
switch (btc_p_debug) do {
    case 0 : {
        btc_debug_log = false;
        btc_debug = false;
        btc_debug_graph = true; // Edited: Allow btc_debug_graph for everyone
...
[
    [ // Edited: Add translation for logistics menu, default = "Fortifications", "Static", "Ammobox", "Containers", "Supplies", "FOB", "Decontamination", "Vehicle Logistic"
    //添加-超跑-固定翼（虚假的）-固定翼（真实的）-直升机（虚假的）-直升机（真实的）-
        "防御工事",
        "载具及固定式武器",
        "弹药箱",
        "集装箱",
    ...
    [
        ...
        [
            //"Containers""集装箱"
            "B_Slingload_01_Cargo_F", // Edited: Add cargo containers
            "Land_Pod_Heli_Transport_04_box_black_F" // Edited: Add cargo containers
        ...
        [
            //"Decontamination""消杀喷淋装置"
            "DeconShower_01_F"
        ],
        [
            //"Vehicle logistic""后勤补给"
            "ACE_Wheel",
            "ACE_Track",
...
//几个缩写-海军（VMF）、MSV-特种部队单位、快速反应部队（RVA Rapid Reaction Forces）、TV-俄罗斯空降兵、VDV-俄罗斯空降兵、空天军-俄罗斯空军防空部队（VPVO Voenno-Vozdushnye Sily i Voyska Protivovozdushnoy Oborony）、特种部队内卫部队（VV）、俄罗斯空军（VVS）

private _allfaction = ["IND_L_F","IND_F","OPF_F","OPF_G_F","IND_G_F","BLU_G_F","IND_E_F","BLU_F","SAB_SW_FACTION_RED","SAB_SW_FACTION_GREEN","SAB_SW_FACTION_BLUE","IND_C_F","EMP_GREYSTONE_PMC_OPFOR","EMP_GREYSTONE_PMC","OPF_R_F","BLU_GEN_F","BLU_T_F","OPF_GEN_F","OPF_T_F","BLU_CTRG_F","RFARMY_EXPEDITIONCORP","RFARMY_EXPEDITION_WINTER_CORP","RFARMY_INFANTRY","RFARMY_SPECOPS","SINIGAMI_SPECIAL_GEC_FORCES","FIR_AWS_FRND_F","FIR_AWS_ENEMY_F","FZA_USAAV","GREATEASTERNCOALITION_REGULAR","JK_B_BAF_F","JK_B_ITA_F","JK_O_TKA_F","JK_B_US_F","JK_B_WOLVES_PMC_F","BLU_W_F","EXPANSION_MOD_GENDARMERIE_CBRN_DIV","PLA_ARMORED_FORCE","RHS_FACTION_VVS_C","RHS_FACTION_VMF","RHS_FACTION_MSV","RHS_FACTION_RVA","RHS_FACTION_TV","RHS_FACTION_VDV","RHS_FACTION_VPVO","RHS_FACTION_VV","RHS_FACTION_VVS","RHS_FACTION_USARMY_D","RHS_FACTION_USARMY_WD","RHS_FACTION_USN","RHS_FACTION_SOCOM","RHS_FACTION_USAF","RHS_FACTION_USMC_D","RHS_FACTION_USMC_WD","RHSGREF_FACTION_CDF_AIR","RHSGREF_FACTION_CDF_AIR_B","RHSSAF_FACTION_AIRFORCE","SAB_FL_FACTION_RED","SAB_FL_FACTION_GREEN","SAB_FL_FACTION_BLUE","UKMARINE_INFANTRY","USAF","WBK_AI_MELEE","WBK_AI","WBK_AI_ZHAMBIES","WEBNIX_CORP_ASSAULTGROUP","WEBNIX_CORP_OFFICIAL","WEBNIX_CORP_SPECOPS","WEST_ASIAN_RADICALS","WEST_ASIAN_RADICALS_SPECOPS"]; //All factions
_p_en = _allfaction select _p_en; //Select faction selected from mission parameter
_allclasse = [[_p_en,"WBK_AI_MELEE","WBK_AI","WBK_AI_ZHAMBIES","WEBNIX_CORP_ASSAULTGROUP","WEBNIX_CORP_OFFICIAL","WEBNIX_CORP_SPECOPS"], _p_en_AA, _p_en_tank] call btc_mil_fnc_class; 
//Create classes from factions, you can combine factions like that: [[_p_en , "IND_F"], _p_en_AA, _p_en_tank] call btc_mil_fnc_class;
...
btc_rep_bonus_mil_killed = 1;// 编辑：增加每个被杀死的敌人的声誉增益,默认 = 0.25
...
btc_rep_malus_player_respawn = 0; // 编辑：玩家重生不再影响声誉,默认 = - 10
btc_rep_malus_veh_killed = 0; // 编辑：玩家车辆被杀死不再影响声誉,默认 = - 25
...
//Door
btc_door_breaking_time = 2; // Edited: Decrease door breaking time, default = 60 撬锁工具破门时间 模拟液压钳 
