...
_action = ["custom_action_recruit", "招募友军步枪手", "", {(group player) createUnit ["rhsusf_usmc_marpat_wd_teamleader", position player, [], 0, "NONE"];}, {count units group player < 6}] call ace_interact_menu_fnc_createAction; 
[btc_gear_object, 0, ["ACE_MainActions"], _action] call ace_interact_menu_fnc_addActionToObject;
...
// Edited: Create Quad Bike at base
_action = ["custom_action_quadbike", "生成超跑", "", {createVehicle ["ageracover10", getMarkerPos "respawn_west", [], 10, "NONE"]
;}, {count (getMarkerPos "respawn_west" nearEntities ["ageracover10", 15]) < 3}] call ace_interact_menu_fnc_createAction;
[btc_gear_object, 0, ["ACE_MainActions"], _action] call ace_interact_menu_fnc_addActionToObject;

_action = ["custom_action_quadbike", "生成AH-6M", "", {createVehicle ["RHS_MELB_AH6M", getMarkerPos "respawn_west", [], 10, "NONE"]
;}, {count (getMarkerPos "respawn_west" nearEntities ["RHS_MELB_AH6M", 15]) < 3}] call ace_interact_menu_fnc_createAction;
[btc_gear_object, 0, ["ACE_MainActions"], _action] call ace_interact_menu_fnc_addActionToObject;
